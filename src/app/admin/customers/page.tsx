        ...
        <CardContent className="p-0">
          <div className="p-4 border-b border-border flex flex-col sm:flex-row gap-3">
            <div className="relative flex-1 max-w-xs">
              ...
              <Input placeholder="Search customers…" className="pl-9 bg-secondary/50" value={search} onChange={e => setSearch(e.target.value)} />
            </div>
              ...
              <tbody className="divide-y divide-border">
                {filtered.length > 0 ? filtered.map(c => (
                  <tr key={c.id} className="hover:bg-secondary/20 transition-colors">
                    ...
                    <td className="px-5 py-4 text-foreground/50 hidden sm:table-cell">{c.joinDate}</td>
                    <td className="px-5 py-4 font-semibold">₹{c.spent.toLocaleString('en-IN')}</td>
                    ...
                    <td className="px-5 py-4">
                      <div className="flex items-center justify-end gap-1">
                        <button onClick={() => openEdit(c)} className="p-1.5 rounded-lg text-foreground/40 hover:text-primary hover:bg-primary/10 transition-colors"><Edit className="w-4 h-4" /></button>
                        <button onClick={() => handleDelete(c.id)} className="p-1.5 rounded-lg text-foreground/40 hover:text-red-400 hover:bg-red-500/10 transition-colors"><Trash2 className="w-4 h-4" /></button>
                      </div>
                    </td>
                  </tr>
                )) : (
